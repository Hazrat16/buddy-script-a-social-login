  ...
  return (
    <div className="mx-auto max-w-xl px-4 py-8 sm:px-6">
        ...
        {loading ? (
          ...
          <p className="mt-10 rounded-2xl border border-dashed border-slate-300 bg-white/80 px-6 py-12 text-center text-sm text-slate-600 dark:border-slate-600 dark:bg-slate-900/50 dark:text-slate-400">
          ...
          <div className="mt-8 space-y-5">
            {posts.map((p) => (
              <PostCard key={p.id} post={p} currentUser={me} onPostUpdated={(next) => setPosts((prev) => prev.map((x) => (x.id === next.id ? next : x)))} onPostDeleted={() => void loadSaved()} />
            ))}
          </div>
        )}
    </div>

...
export function FindFriendsClient() {
    ...
    return (<div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <div className="relative mt-6">
          ...
          <input type="search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by name or email" className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm outline-none focus:border-indigo-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"/>
        </div>

        <ul className="mt-8 space-y-3">
          {loading ? (<li className="text-center text-sm text-slate-500">Loading…</li>) : users.length === 0 ? (<li className="text-center text-sm text-slate-500">No users match that search.</li>) : (users.map((u) => (<li key={u.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
                <UserAvatar user={u} size={52}/>
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-slate-900 dark:text-white">{displayName(u)}</p>
                  <p className="truncate text-xs text-slate-500 dark:text-slate-400">{u.email}</p>
                </div>
                <button type="button" className="shrink-0 rounded-full bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-500" onClick={() => showComingSoon("Connect / Follow")}>
                  Connect
                </button>
              </li>)))}
        </ul>

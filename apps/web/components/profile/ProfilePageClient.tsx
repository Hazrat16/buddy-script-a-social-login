    ...
    return (<div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
            ...
            <div className="relative -mt-12 flex flex-col items-start sm:-mt-14 sm:flex-row sm:items-end sm:gap-6">
              ...
              <div className="mt-4 text-left sm:mb-2 sm:mt-0 sm:flex-1">
                ...
                <div className="mt-3 flex flex-wrap justify-start gap-2">
                  {typeof me.postCount === "number" ? (<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                      {me.postCount} {me.postCount === 1 ? "post" : "posts"}
                    </span>) : null}
                  {typeof me.commentCount === "number" ? (<span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                      {me.commentCount} {me.commentCount === 1 ? "comment" : "comments"}
                    </span>) : null}
                  {memberSince ? (<span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-800 dark:bg-indigo-950/60 dark:text-indigo-200">
                      Member since {memberSince}
                    </span>) : null}
                </div>
                <Link href="/settings" className="mt-4 inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-800 shadow-sm transition hover:border-indigo-200 hover:bg-indigo-50/80 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:hover:border-indigo-800 dark:hover:bg-indigo-950/40">
                  Account settings
                </Link>
              </div>
            </div>
        ...
        <section className="mt-8" aria-labelledby="profile-posts-heading">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3 border-b border-slate-200 pb-3 dark:border-slate-800">
            ...
            <Link href="/feed" className="text-sm font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300">
              Write a post
            </Link>
          </div>

          {postsErr ? (<p className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-200">
              {postsErr}
            </p>) : null}

          {postsLoading ? (<FeedSkeleton />) : myPosts.length === 0 ? (<div className="rounded-2xl border border-dashed border-slate-300 bg-white/80 px-6 py-12 text-center dark:border-slate-600 dark:bg-slate-900/40">
              ...
              <Link href="/feed" className="mt-5 inline-flex items-center justify-center rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-500">
                Go to feed
              </Link>
            </div>) : (<div className="space-y-5">
              {myPosts.map((p) => (<PostCard key={p.id} post={p} currentUser={me} onPostUpdated={(next) => {
                    setMyPosts((prev) => prev.map((x) => (x.id === next.id ? next : x)));
                }}/>))}
            </div>)}

          {!postsLoading && postsCursor ? (<div className="mt-8 flex justify-center">
              <button type="button" className="rounded-xl border border-slate-200 bg-white px-6 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-slate-600" disabled={postsLoadingMore} onClick={() => void loadMorePosts()}>

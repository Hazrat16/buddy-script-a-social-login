...
export function FeedRightAside({ users }: {
    users: PublicUser[];
}) {
    ...
    return (<div className="space-y-4">
      <div className="rounded-2xl border border-slate-200/90 bg-white/90 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
        ...
        {might.map((u) => ignored[u.id] ? null : (<div key={u.id} className="mb-4 last:mb-0">
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button type="button" className="rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800" onClick={() => setIgnored((m) => ({ ...m, [u.id]: true }))}>
                      Ignore
                    </button>
                    <button type="button" className={`rounded-full px-3 py-1 text-xs font-bold transition ${followed[u.id]
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white/90 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <div className="relative mb-3">
          ...
          <input type="search" readOnly placeholder="Search friends (soon)" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm text-slate-500 outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400"/>
        </div>
        <ul className="max-h-[320px] space-y-1 overflow-y-auto pr-1">
          {friends.map((u) => {
            const on = onlineDot(u.id);
            return (<li key={u.id}>
                <Link href="/messages" className="flex items-center justify-between gap-2 rounded-xl px-2 py-2 transition hover:bg-slate-50 dark:hover:bg-slate-800/80">
                  ...
                  {on ? (<span className="h-2.5 w-2.5 shrink-0 rounded-full bg-emerald-500 ring-2 ring-white dark:ring-slate-900" title="Online"/>) : (<span className="shrink-0 text-[10px] text-slate-400">5m</span>)}
                </Link>
              </li>);
        })}
        </ul>

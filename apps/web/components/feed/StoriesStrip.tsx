...
const demoStories: { name: string; seen: boolean }[] = [
...
export function StoriesStrip({ me }: { me: PublicUser }) {
  return (
    <div className="mb-6">
      <div className="hidden md:block">
        <div className="relative rounded-2xl border border-slate-200/90 bg-white/90 p-4 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
          <div className="grid grid-cols-4 gap-3">
            <Link
              href="/feed"
              className="group relative aspect-[3/4] overflow-hidden rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 ring-2 ring-indigo-200 ring-offset-2 dark:ring-indigo-900 dark:ring-offset-slate-900"
            >
            ...
            {demoStories.slice(1).map((s) => (
              <button
                key={s.name}
                type="button"
                className={`relative aspect-[3/4] overflow-hidden rounded-xl bg-slate-200 transition hover:opacity-95 dark:bg-slate-800 ${
                  s.seen ? "ring-1 ring-slate-300 dark:ring-slate-600" : "ring-2 ring-indigo-400 ring-offset-2 dark:ring-offset-slate-900"
                }`}
                title={s.name}
              >
      ...
      <div className="-mx-1 flex gap-3 overflow-x-auto pb-2 md:hidden">
        ...
        {demoStories.slice(1).map((s) => (
          <button key={s.name} type="button" className="flex w-16 shrink-0 flex-col items-center gap-1">

...
function ExploreIcon({ kind }: {
    kind: "compass" | "chart" | "users" | "bookmark" | "group" | "game" | "gear" | "save";
}) {
    const cls = "h-4 w-4 shrink-0 text-slate-500 dark:text-slate-400";
    switch (kind) {
        case "compass":
            return (<svg className={cls} fill="currentColor" viewBox="0 0 20 20">
        ...
        case "chart":
            return (<svg className={cls} fill="currentColor" viewBox="0 0 22 24">
        ...
        case "users":
            return (<svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        case "bookmark":
            return (<svg className={cls} fill="none" viewBox="0 0 22 24" stroke="currentColor" strokeWidth={2}>
        ...
        case "game":
            return (<svg className={cls} fill="currentColor" viewBox="0 0 24 24">
...
export function FeedLeftAside({ suggested }: {
    ...
    return (<div className="space-y-4">
      <div className="rounded-2xl border border-slate-200/90 bg-white/90 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <ul className="mt-4 space-y-0.5">
          {explore.map((item) => (<li key={item.label}>
              {item.href === "#" ? (<button type="button" className="flex w-full items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800" onClick={() => showComingSoon(item.label)}>
                  <span className="flex min-w-0 items-center gap-2">
                    <ExploreIcon kind={item.icon}/>
                    <span className="truncate">{item.label}</span>
                  </span>
                  {item.badge ? (<span className="shrink-0 rounded-md bg-indigo-100 px-1.5 py-0.5 text-[10px] font-bold uppercase text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300">
                      {item.badge}
                    </span>) : null}
                </button>) : (<Link href={item.href} className="flex items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-slate-800">
                  <span className="flex min-w-0 items-center gap-2">
                    <ExploreIcon kind={item.icon}/>
                    <span className="truncate">{item.label}</span>
                  </span>
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white/90 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <ul className="space-y-4">
          {suggested.slice(0, 4).map((u) => (<li key={u.id} className="flex items-center gap-3">
              ...
              <Link href="/find-friends" className="shrink-0 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700 transition hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-950/60 dark:text-indigo-300">
                Connect
              </Link>
            </li>))}
      ...
      <div className="rounded-2xl border border-slate-200/90 bg-white/90 p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900/90">
        ...
        <button type="button" className="block w-full overflow-hidden rounded-xl border border-slate-100 text-left transition hover:border-indigo-200 dark:border-slate-800 dark:hover:border-indigo-900" onClick={() => showComingSoon("Events")}>

    ...
    const t = window.setTimeout(() => {
      try {
        ...
        localStorage.setItem(
          DRAFT_KEY,
          JSON.stringify({ body, visibility: "PUBLIC" }),
        );
      } catch {}
    }, 450);
    ...
    return (
      <div className="_feed_inner_text_area _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
        {draftRestored && body.trim() ? (
          <p className="_notification_para" style={{ marginBottom: 12 }}>
            Restored a saved draft from this browser.
          </p>
        ) : null}
        <div className="_feed_inner_text_area_box">
          <div className="_feed_inner_text_area_box_image">
            <UserAvatar user={me} size={40} shape="rounded-full" />
          </div>
          <div className="form-floating _feed_inner_text_area_box_form">
            <textarea
              className={`form-control _textarea${hasComposerText ? " buddy-composer-has-text" : ""}`}
            ...
            />
            <label
              className="_feed_textarea_label flex"
              htmlFor="buddy-composer-textarea"
            >
              Write something ...
              {svgComposerPencil}
            </label>
        ...
        {nearLimit ? (
          <p
            ...
            style={{ marginTop: 8, color: "#b45309", fontWeight: 600 }}
          >
    ...
    <div className="mb-6 rounded-2xl border border-slate-200/90 bg-white/95 p-5 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:p-6">
      ...
      {file ? (
        <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">
          ...
          <button
            type="button"
            className="ml-1 font-semibold text-indigo-600 hover:underline dark:text-indigo-400"
            onClick={() => setFile(null)}
          >
            Remove
          </button>

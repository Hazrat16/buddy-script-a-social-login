...
export function BuddyAppFrame({ user, children, }: {
    ...
    const { items: notifItems, setItems: setNotifItems, bump } = useNotifList();
    const notifRef = useRef<HTMLSpanElement>(null);
    const notifMobileBtnRef = useRef<HTMLButtonElement>(null);
    const notifMobilePanelRef = useRef<HTMLDivElement>(null);
    ...
    useEffect(() => {
        function onDoc(e: MouseEvent) {
            ...
            const inDesktopNotif = notifRef.current?.contains(t);
            const inMobileNotif = notifMobileBtnRef.current?.contains(t) || notifMobilePanelRef.current?.contains(t);
    ...
    const unread = useMemo(() => notifItems.filter((n) => !n.read).length, [notifItems]);
    ...
    function markAllRead() {
        const next = markAllNotificationsRead(notifItems);
        setNotifItems(next);
        persistNotifications(next);
        bump();
        window.dispatchEvent(new CustomEvent("buddy:notifications-changed"));
        setNotifSubOpen(false);
    }
    function notifPanelBody() {
        ...
        <div className="_notifications_content">
          ...
          <div className="_notification_box_right">
            <button type="button" className="_notification_box_right_link" onClick={(e) => {
                e.stopPropagation();
                setNotifSubOpen((s) => !s);
            }}>
            ...
            <div className={`_notifications_drop_right${notifSubOpen ? " show" : ""}`}>
              <ul className="_notification_list">
                ...
                <li className="_notification_item">
                  <Link href="/settings" className="_notification_link" onClick={() => setNotifSubOpen(false)}>
        ...
        <div className="_notifications_drop_box">
          ...
          <div className="_notifications_all">
            {notifItems.slice(0, 8).map((n) => (<div key={n.id} className="_notification_box">
                <div className="_notification_image">
                  <img src={PLACEHOLDER_IMG} alt="" className="_notify_img"/>
                </div>
    ...
    return (<div className={`_layout _layout_main_wrapper${dark ? " _dark_wrapper" : ""}`}>
      <div className="_layout_mode_swithing_btn">
        <button type="button" className="_layout_swithing_btn_link" aria-pressed={dark} aria-label={dark ? "Switch to light mode" : "Switch to dark mode"} onClick={() => setDark((d) => !d)}>
      ...
      <div className="_main_layout">
          ...
          <div className="container _custom_container">
            <div className="_logo_wrap">
              <Link className="navbar-brand" href="/feed">
                <img src="/assets/images/logo.svg" alt="Buddy" className="_nav_logo"/>
              </Link>
            </div>
            <button className="navbar-toggler bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
              <span className="navbar-toggler-icon"/>
            </button>
            <div className="collapse navbar-collapse buddy-navbar-collapse" id="navbarSupportedContent">
              <div className="_header_form buddy-navbar-search">
                <form className="_header_form_grp" onSubmit={(e) => {
            e.preventDefault();
            const fd = new FormData(e.currentTarget);
            const q = String(fd.get("q") ?? "").trim();
            router.push(q ? `/find-friends?q=${encodeURIComponent(q)}` : "/find-friends");
        }}>
                  <svg className="_header_form_svg" xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="none" viewBox="0 0 17 17">
                    <circle cx="7" cy="7" r="6" stroke="#666"/>
                    <path stroke="#666" strokeLinecap="round" d="M16 16l-3-3"/>
                  </svg>
                  <input className="form-control me-2 _inpt1" name="q" type="search" placeholder="Search…" aria-label="Search"/>
                </form>
              </div>
              <div className="buddy-navbar-actions">
              <ul className="navbar-nav mb-2 mb-lg-0 _header_nav_list _mar_r8">
                <li className="nav-item _header_nav_item">
                  <Link className={`nav-link _header_nav_link${feedActive ? " _header_nav_link_active" : ""}`} aria-current={feedActive ? "page" : undefined} href="/feed">
                ...
                <li className="nav-item _header_nav_item">
                  <Link className="nav-link _header_nav_link" href="/find-friends">
                ...
                <li className="nav-item _header_nav_item">
                  <span ref={notifRef} className="nav-link _header_nav_link _header_notify_btn" onClick={(e) => {
            e.stopPropagation();
            setNotifOpen((v) => !v);
        }} onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
                ...
                setNotifOpen((v) => !v);
                    ...
                    {unread > 0 ? <span className="_counting">{unread > 99 ? "99+" : unread}</span> : null}
                ...
                <li className="nav-item _header_nav_item">
                  <Link className="nav-link _header_nav_link" href="/messages">
              ...
              <div className="_header_nav_profile" ref={profileRef}>
                <div className="_header_nav_profile_image">
                  <UserAvatar user={user} size={40} shape="rounded-full"/>
                </div>
                <div className="_header_nav_dropdown">
                  <p className="_header_nav_para">{name}</p>
                  <button id="_profile_drop_show_btn" type="button" className="_header_nav_dropdown_btn _dropdown_toggle" aria-expanded={profileOpen} onClick={(e) => {
            e.stopPropagation();
            setProfileOpen((v) => !v);
        }}>
                ...
                <div id="_prfoile_drop" className={`_nav_profile_dropdown _profile_dropdown${profileOpen ? " show" : ""}`}>
                  <div className="_nav_profile_dropdown_info">
                    <div className="_nav_profile_dropdown_image">
                      <UserAvatar user={user} size={54} shape="rounded-full" className="shrink-0 ring-2 ring-[#377DFF] ring-offset-0 shadow-none"/>
                    </div>
                    <div className="_nav_profile_dropdown_info_txt">
                      <h4 className="_nav_dropdown_title">{name}</h4>
                      <Link href="/profile" className="_nav_drop_profile" onClick={() => setProfileOpen(false)}>
                        View Profile
                      </Link>
                    </div>
                  </div>
                  ...
                  <ul className="_nav_dropdown_list">
                        ...
                        <div className="_nav_drop_info">
                          <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="19" fill="none" viewBox="0 0 18 19">
                              <path fill="#377DFF" d={PROFILE_MENU_SETTINGS_PATH}/>
                            </svg>
                          </span>
                          Settings
                        </div>
                    ...
                    <li className="_nav_dropdown_list_item">
                      <Link href="/messages" className="_nav_dropdown_link" onClick={() => setProfileOpen(false)}>
                        <div className="_nav_drop_info">
                          <span>
                    ...
                    <li className="_nav_dropdown_list_item">
                      <button type="button" className="_nav_dropdown_link" onClick={() => {
            setProfileOpen(false);
            void logout();
        }}>
                        <div className="_nav_drop_info">
                    ...
                    <div className="_header_mobile_menu_top_inner">
                      ...
                      <div className="_header_mobile_menu_right">
                        <form className="_header_form_grp">
                          <Link href="/find-friends" className="_header_mobile_search">
        ...
        {notifOpen ? (<div ref={notifMobilePanelRef} className="_notification_dropdown show d-lg-none" style={{ position: "relative", zIndex: 40 }}>
            <div style={{ margin: "0 12px 12px", maxHeight: "70vh", overflow: "auto" }}>{notifPanelBody()}</div>
          </div>) : null}
                ...
                <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                  <ul className="_mobile_navigation_bottom_list">
                    <li className="_mobile_navigation_bottom_item">
                      <Link href="/feed" className={`_mobile_navigation_bottom_link${feedActive ? " _mobile_navigation_bottom_link_active" : ""}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="27" fill="none" viewBox="0 0 24 27">
                          <path className="_mobile_svg" fill="#000" fillOpacity=".6" stroke="#666" strokeWidth="1.5" d="M1 13.042c0-2.094 0-3.141.431-4.061.432-.92 1.242-1.602 2.862-2.965l1.571-1.321C8.792 2.232 10.256 1 12 1c1.744 0 3.208 1.232 6.136 3.695l1.572 1.321c1.62 1.363 2.43 2.044 2.86 2.965.432.92.432 1.967.432 4.06v6.54c0 2.908 0 4.362-.92 5.265-.921.904-2.403.904-5.366.904H7.286c-2.963 0-4.445 0-5.365-.904C1 23.944 1 22.49 1 19.581v-6.54z"/>
                          <path fill="#fff" stroke="#fff" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.07 18.497h5.857v7.253H9.07v-7.253z"/>
                        </svg>
                      </Link>
                    </li>
                    <li className="_mobile_navigation_bottom_item">
                      <Link href="/find-friends" className="_mobile_navigation_bottom_link">
                    ...
                    <li className="_mobile_navigation_bottom_item">
                      <button ref={notifMobileBtnRef} type="button" className="_mobile_navigation_bottom_link" style={{ border: "none", background: "transparent", width: "100%" }} onClick={(e) => {
            e.stopPropagation();
            setNotifOpen((v) => !v);
        }}>
                    ...
                    <li className="_mobile_navigation_bottom_item">
                      <Link href="/messages" className="_mobile_navigation_bottom_link">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    ...
                    <div className="_header_mobile_toggle">
                      <Link href="/settings" className="_header_mobile_btn_link">

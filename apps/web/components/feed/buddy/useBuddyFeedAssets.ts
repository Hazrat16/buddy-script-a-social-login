...
export function useBuddyFeedAssets(enabled: boolean) {
    useEffect(() => {
        if (!enabled)
            return;
        ...
        ensureLink(`${PREFIX}-buddy-fixes`, "/assets/css/buddy-feed-fixes.css", "stylesheet");
        ...
        document.body.classList.add("buddy-feed-active");
        return () => {
            document.body.classList.remove("buddy-feed-active");
            for (const id of [
                `${PREFIX}-font-pre1`,
                `${PREFIX}-font-pre2`,
                `${PREFIX}-font`,

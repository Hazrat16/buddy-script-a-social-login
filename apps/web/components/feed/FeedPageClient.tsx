...
export function FeedPageClient() {
    ...
    return (<BuddyFeedShell directoryUsers={directoryUsers}>
      <BuddyStoriesStrip me={me}/>
      <PostComposer me={me} buddySkin onPosted={(newPost) => setPosts((prev) => [newPost, ...prev])}/>
      {err ? (<div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16 _padd_r24 _padd_l24">
          <p className="_notification_para" style={{ color: "#c00" }}>
            {err}
          </p>
        </div>) : null}

      {postsLoading ? (<div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16 _padd_r24 _padd_l24">
          <FeedSkeleton />
        </div>) : visiblePosts.length === 0 ? (<div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16 _padd_r24 _padd_l24">
          ...
          {visiblePosts.map((p) => (<PostCard key={p.id} post={p} currentUser={me} buddySkin onPostUpdated={(next) => setPosts((prev) => prev.map((x) => (x.id === next.id ? next : x)))} onPostDeleted={(id) => setPosts((prev) => prev.filter((x) => x.id !== id))} onPostHidden={() => setHiddenTick((t) => t + 1)}/>))}
        </>)}

      {!postsLoading && cursor ? (<div className="_mar_b16" style={{ textAlign: "center" }}>

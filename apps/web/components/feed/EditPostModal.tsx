...
export function EditPostModal({ open, onClose, post, currentUser, onSaved }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-slate-900/60 p-4 pt-10 backdrop-blur-sm sm:pt-16"
      role="presentation"
      onMouseDown={(e) => {
    ...
    >
      <div
        className="w-full max-w-[500px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-[#242526]"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
          ...
          <button
            type="button"
            className="rounded-full p-2 text-slate-500 hover:bg-slate-100 dark:text-[#b0b3b8] dark:hover:bg-slate-700"
            aria-label="Close"
            onClick={onClose}
          >
          ...
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:bg-[#3a3b3c] dark:text-[#e4e6eb]">
                ...
                <select
                  id="edit-visibility"
                  className="cursor-pointer bg-transparent font-semibold outline-none dark:text-[#e4e6eb]"
                  value={visibility}
        ...
        <div className="px-4 py-3">
          <textarea
            className="min-h-[140px] w-full resize-y bg-transparent text-[15px] leading-relaxed text-slate-900 outline-none placeholder:text-slate-400 dark:text-[#e4e6eb] dark:placeholder:text-[#8a8d91]"
          ...
          <button
            type="button"
            className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-[#b0b3b8] dark:hover:bg-[#3a3b3c]"
        ...
        {err ? (
          <p className="border-t border-red-100 bg-red-50 px-4 py-2 text-sm text-red-800 dark:border-red-900/40 dark:bg-red-950/40 dark:text-red-200">
            {err}
          </p>
        ) : null}
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 px-4 py-3 dark:border-slate-700">
          <button
            type="button"
            className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 dark:text-[#e4e6eb] dark:hover:bg-[#3a3b3c]"
            onClick={onClose}
            disabled={saving}
          >
            Cancel
          ...
          <button
            type="button"
            className="rounded-lg bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50 dark:bg-[#2374e1] dark:hover:bg-[#1877f2]"
            onClick={() => void save()}
            disabled={saving}
          >
            {saving ? "Saving…" : "Save"}
